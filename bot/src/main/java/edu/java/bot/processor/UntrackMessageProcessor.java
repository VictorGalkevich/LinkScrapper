...
public class UntrackMessageProcessor implements UserMessageProcessor {
    ...
    public SendMessage process(Update update) {
        ...
        if (s.length != 2) {
            message = "/untrack syntax might be [/untrack <link>]";
        } else if (!isValid(s[1])) {
            message = "Provided link is invalid";
        } else if (!isTracked(parse(s[1]), id)) {
            message = "Link is not being tracked";
        } else {
            Optional<User> user = userRepository.findById(id);
            user.ifPresent(it -> it.getLinks().remove(parse(s[1])));
            message = "Link was removed from tracking list!";
        }
